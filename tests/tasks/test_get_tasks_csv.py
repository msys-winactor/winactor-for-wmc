...
def test_run_calls_get_csv(mocker):
    ...
    mock_client_class = mocker.patch(
        "winactor_for_wmc.tasks.get_tasks_csv.WMCApiClient"
    )
    mock_client = mock_client_class.return_value
    mock_client.get_csv.return_value = None  # runでは返り値は使われない
    ...
    # run関数を呼ぶ
    result = get_tasks_csv.run(**kwargs)
    ...
    mock_client.get_csv.assert_called_once_with(
        "/tasks/csv", params={"foo": "bar"}, save_path="/tmp/test.csv"
    )
