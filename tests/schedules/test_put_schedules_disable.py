...
def test_run_calls_put_disable(mocker):
    # WMCApiClient クラスをモック化
    mock_client_class = mocker.patch(
        "winactor_for_wmc.schedules.put_schedules_disable.WMCApiClient"
    )
    mock_client = mock_client_class.return_value
    mock_client.put.return_value = {"result": "OK"}

    result = put_schedules_disable.run(
    ...
    mock_client.put.assert_called_once_with("/schedules/sched_001/disable")
