# tests/schedules/test_put_schedules.py
...
def test_run_all_departments(mocker):
    gd = mocker.patch("winactor_for_wmc.schedules.put_schedules.get_departments")
    client_cls = mocker.patch("winactor_for_wmc.schedules.put_schedules.WMCApiClient")
...
def test_run_partial_departments_only_parent(mocker):
    ...
    client.put.assert_called_once_with("/schedules/456", data={"department1": "1"})
...
def test_run_only_department2(mocker):
    ...
    client.put.assert_called_once_with("/schedules/789", data={"department2": "2"})
    assert schedule_data == {}  # in-place 更新なし
    assert result == {"result": "OK"}
...
def test_run_only_department3(mocker):
    ...
    client.put.assert_called_once_with("/schedules/999", data={"department3": "3"})
    assert schedule_data == {}  # in-place 更新なし
    assert result == {"result": "OK"}
...
def test_run_with_department_names_but_no_match_adds_nothing(mocker):
    ...
    client.put.assert_called_once_with(
        "/schedules/no_match", data={"name": "test_schedule"}
    )
...
def test_run_whitespace_name_triggers_lookup_but_adds_nothing(mocker):
    ...
    client.put.assert_called_once_with(
        "/schedules/whitespace", data={"name": "test_schedule2"}
    )
    assert result == {"result": "OK"}
...
def test_run_ids_zero_and_empty_string_are_posted(mocker):
    ...
    client.put.assert_called_once_with(
        "/schedules/zero_empty", data={"department1": 0, "department2": ""}
    )
...
def test_run_overwrites_existing_department_fields(mocker):
    ...
    client.put.assert_called_once_with(
        "/schedules/overwrite",
        data={"department1": "old1", "department2": "22", "name": "keep_schedule"},
    )
    assert schedule_data == {
        "department1": "old1",
        "department2": "22",
...
def test_run_no_departments_with_non_empty_schedule_data_inplace_update(mocker):
    ...
    mocker.patch("winactor_for_wmc.schedules.put_schedules.get_departments")
    client_cls = mocker.patch("winactor_for_wmc.schedules.put_schedules.WMCApiClient")
    client = client_cls.return_value
    client.put.return_value = {"result": "OK"}
    ...
    client.put.assert_called_once_with("/schedules/inplace", data=expected_data)
    # in-place 反映されることを確認
    assert schedule_data == expected_data
    assert result == {"result": "OK"}
