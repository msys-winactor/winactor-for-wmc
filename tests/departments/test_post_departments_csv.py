...
def test_run_with_all_import_data(mocker):
    client_cls = mocker.patch(
        "winactor_for_wmc.departments.post_departments_csv.WMCApiClient"
    )
    ...
    result = post_departments_csv.run(
        base_url="https://example.com",
        token="dummy_token",
        csv_file_path="/path/to/departments.csv",
    ...
    assert endpoint == "/departments/csv"
...
def test_run_with_delete_execute_type(mocker):
    ...
    result = post_departments_csv.run(
        base_url="https://example.com",
        token="delete_token",
        csv_file_path="/path/to/delete_departments.csv",
        import_data=import_data,
    )
...
def test_run_file_handling(mocker):
    ...
    client = client_cls.return_value
    mock_file = mock_open(
        read_data=b"department1,department2,department3,description\nA,B,C,test"
    )
    mocker.patch("builtins.open", mock_file)
