...
def test_run_calls_get_csv(mocker):
    ...
    mock_client_class = mocker.patch(
        "winactor_for_wmc.departments.get_departments_csv.WMCApiClient"
    )
    ...
    # run関数を呼ぶ
    result = get_departments_csv.run(**kwargs)
    ...
    mock_client.get_csv.assert_called_once_with(
        "/departments/csv", params={"foo": "bar"}, save_path="/tmp/test.csv"
    )
