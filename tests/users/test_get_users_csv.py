from winactor_for_wmc.users import get_users_csv


def test_run_calls_get_csv(mocker):
    # WMCApiClientクラスをモック化
    mock_client_class = mocker.patch(
        "winactor_for_wmc.users.get_users_csv.WMCApiClient"
    )
    ...
    # run関数を呼ぶ
    result = get_users_csv.run(**kwargs)
    ...
    mock_client.get_csv.assert_called_once_with(
        "/users/csv", params={"foo": "bar"}, save_path="/tmp/test.csv"
    )
