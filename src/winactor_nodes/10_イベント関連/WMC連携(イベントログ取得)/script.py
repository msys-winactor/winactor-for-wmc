...
if __name__ == "__main__":
        ...
        # ソート・ページング（PAGE は削除、SIZE は固定 100）
        ...
        raw_params = {
            ...
            # "page" は渡さない
            "size": SIZE,  # 固定値
            # departments_url は渡さず BASE_URL を利用（モジュール側で自動的に base_url を使用）
        }

        # 特例: 所属(親) に「共有」、所属(子) と 所属(孫) が空欄なら
        # department1="0" を明示設定し、department2/3 は未設定のままにする
        if (_norm(DEPARTMENT_NAME1) == "共有") and (not _norm(DEPARTMENT_NAME2)) and (not _norm(DEPARTMENT_NAME3)):
            raw_params["department1"] = "0"
            # 名前は残っていてもモジュール側は department1 があるためID解決をスキップする
        ...
        # 取得結果を WinActor 変数に格納（必要なものを選択して利用）
