...
if CRONSTARTYEARMONTH:
    CRONSTARTYEARMONTH = CRONSTARTYEARMONTH.strip()
    if CRONSTARTYEARMONTH:
        schedule_dict["cronStartYearMonth"] = CRONSTARTYEARMONTH

if CRONENDYEARMONTH:
    CRONENDYEARMONTH = CRONENDYEARMONTH.strip()
    if CRONENDYEARMONTH:
        schedule_dict["cronEndYearMonth"] = CRONENDYEARMONTH

if CRON:
    CRON = CRON.strip()
    if CRON:
        schedule_dict["cron"] = CRON
...
try:
    ...
    raise winactor.WinActorError(1, f"スケジュール登録(条件指定)エラー\n{str(e)}")  # type: ignore